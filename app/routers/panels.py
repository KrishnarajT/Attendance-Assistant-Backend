...
def add_panel( panel: Panel):
    ...
    """
    try:
        add_panel_service = AssistanceService()
        added_panel = add_panel_service.add_panel(panel)
        if added_panel:
            return added_panel
        else:
            raise HTTPException(status_code=500, detail="An error occurred while adding the panel")
...
def get_all_panels():
    ...
    """
    try:
        get_all_panels_service = AssistanceService()
        all_panels = get_all_panels_service.get_all_panels()
        if all_panels:
            return all_panels
        else:
            raise HTTPException(status_code=500, detail="An error occurred while getting all panels")
    except PyMongoError as e:
        raise HTTPException(status_code=500, detail=str(e))  
